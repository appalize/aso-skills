...
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/categories/6014/downloads-to-top?country=us&chart=top-free"
```
...
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/categories/6013/top?country=us&chart=top-free&limit=10"

# 2. How many downloads to compete?
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/categories/6013/downloads-to-top?country=us&chart=top-free"
...
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/new-releases?country=us&maxDays=14"

# 5. What's Apple featuring?
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/featured?country=us"
```
...
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/market/movers?country=us&chart=top-free&genre=$GENRE_ID&limit=10"

# Full activity feed
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/market/activity?country=us&chart=top-free&genre=$GENRE_ID"

# Your category chart position
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/apps/$APP_ID/country-rankings?chart=top-free"

# Downloads needed to move up
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/categories/$GENRE_ID/downloads-to-top?country=us"

# New competitors entering the market
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/new-releases?country=us&maxDays=7"

# Any new #1 apps in your category?
curl -H "X-API-Key: $KEY" \
  "https://api.appalize.com/v1/discover/new-number-1?country=us"
```
