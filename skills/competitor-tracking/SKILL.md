...
Use Appalize to identify competitors if unknown:
```bash
...
Set a calendar reminder. Run the Appalize API calls above. Fill the template.

### Semi-automated

Build a script that calls Appalize weekly and diffs results:
...
for APP_ID in "${APPS[@]}"; do
  echo "=== $APP_ID ==="
  curl -s "https://api.appalize.com/v1/apps/$APP_ID" \
